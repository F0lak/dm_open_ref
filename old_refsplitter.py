...
def move_see_also(text: str) -> str:
	'''
		Moves the "See Also" section to the bottom of the file
	'''
	start: str = "**See also:**"
	start_index: int = text.find(start)
	i: int = start_index
	end_index = None
	while i < len(text):
		if text[i] == "\n":
...
def prep_html_file(text: str) -> str:
	'''
...
class Page:
	...
	'''
	def __init__ (self, path: str, title: str, extension: str, text: str): 
