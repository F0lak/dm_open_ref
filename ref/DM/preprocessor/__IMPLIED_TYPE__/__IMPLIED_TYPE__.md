...
the [`new` proc](/ref/proc/new/new.md) and assigning to a var, the type path for    
`new()` is implied by the var\'s type. Implied types are also    
automatically used in [locate()](/ref/proc/locate/locate.md){.code}, and are used by    
default for the second argument in [istype()](/ref/proc/istype/istype.md){.code}.    
### Example:    
...
-   Within the second argument of [istype()](/ref/proc/istype/istype.md){.code}.    
