...
by the [`#warn` directive](/ref/DM/preprocessor/warn.md)). The warning name    
...
turned on for linting purposes:    
  ...
  frequent_call   A very commonly called proc such as [New()](/ref/datum/proc/New.md){.code} or [Del()](/ref/datum/proc/Del.md){.code} has been overridden on a type that gets created or destroyed frequently, such as `/turf`, `/atom`, or `/datum`.  