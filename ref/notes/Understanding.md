...
Every atom has an [appearance](/ref/atom/var/appearance/appearance.md) that holds all of    
...
[icon](/ref/atom/var/icon/icon.md){.code} and    
[icon_state](/ref/atom/var/icon_state/icon_state.md){.code} and produce only a single    
sprite, some are more complex with overlays, underlays, maptext, etc.    
Also there may be [image objects](/ref/image/image.md) and [visual    
contents](/ref/atom/var/vis_contents/vis_contents.md) involved, although they\'re not part    
...
[direction](/ref/atom/var/dir/dir.md) to draw and which frame of the icon\'s    
...
:::    
::::::::::    
The [underlays](/ref/atom/var/underlays/underlays.md){.code} list is processed first,    
then [overlays](/ref/atom/var/overlays/overlays.md){.code}. These lists contain    
...
Any atom can have an [image object](/ref/image/image.md) attached, which can be    
shown to only specific players. Most atoms, and image objects, can have    
[visual contents](/ref/atom/var/vis_contents/vis_contents.md) that display other atoms as    
if they\'re overlays.    
:::::::: renderlist    
::: renderlist-box    
Underlays    
...
-   If an image object uses the [override](/ref/atom/var/override/override.md){.code}    
...
-   An object in visual contents can use    
    [vis_flags](/ref/atom/var/vis_flags/vis_flags.md){.code} to set `VIS_UNDERLAY` and    
    move itself before the parent\'s underlays.    
### Maptext and particles    
Any appearance may have [maptext](/ref/atom/var/maptext/maptext.md){.code} attached.    
...
An appearance\'s [color](/ref/atom/var/color/color.md){.code} and    
[alpha](/ref/atom/var/alpha/alpha.md){.code} vars (from here forwarded they\'ll just    
be referred to by `color`) and [transform](/ref/atom/var/transform/transform.md){.code}    
...
special [appearance_flags](/ref/var/appearance_flags/appearance_flags.md){.code}:    
...
[appearance_flags](/ref/var/appearance_flags/appearance_flags.md){.code} (called KT for short),    
...
[plane](/ref/atom/var/plane/plane.md){.code} than the group\'s owner, it will be    
separated as if it defined `KEEP_APART`, except it can escape multiple    
nested groups.    
### Layers and planes    
Any appearance can have a [layer](/ref/atom/var/layer/layer.md){.code} or    
[plane](/ref/atom/var/layer/layer.md){.code}, and these influence how it gets    
sorted. (There\'s also a concept called a \"sub-plane\" that\'s    
influenced by whether an atom is a [HUD/screen    
object](/ref/%7Bnotes%7D/HUD/HUD.md) or special layers like    
[BACKGROUND_LAYER](/ref/%7Bnotes%7D/BACKGROUND_LAYER/BACKGROUND_LAYER.md){.code}.)    
...
2.  Subplane is counted next. E.g., HUD objects render above non-HUD    
    objects.    
3.  Depending on [world.map_format](/ref/world/var/map_format/map_format.md){.code},    
...
[appearance_flags](/ref/var/appearance_flags/appearance_flags.md){.code} has a value called    
...
targets](/ref/atom/var/render_target/render_target.md) and special map formats. Any details    
