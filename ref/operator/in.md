...
The `in` operator is also a modifier for some procs such as <a class="code" href="#/proc/locate">locate()</a> and <a class="code" href="#/proc/input">input()</a>.


> [!TIP]
> Note: For <a href="#/list/associations">associative lists</a> there's a faster way to see if an item is in that list. The lookup of `List[A]` in an associative list is relatively fast, so if the associated value is always expected to be true (not null, 0, or an empty string), you can use `List[A]` instead of `A in List` in those situations.
***
**Related Pages:**
+    [list](/ref/list)
