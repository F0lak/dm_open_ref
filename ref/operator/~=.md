...
-   Two [/matrix](/ref/matrix.md) -m values are equivalent if their components
    are the same.
-   Datums A and B are equivalent if A
    [overloads](/ref/operator/overload.md) -m the \~= operator with a proc that
...
values](/ref/list/assoc.md) -min lists are also checked for equality. This was
