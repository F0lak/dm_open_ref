...
instance, will start slow, accelerate very quickly in the middle, and
slow down again at the end. A sine curve could be used with a flip
transformation to make a coin appear to spin. A text bubble can jump
...
animation sequence with the same name, but leaving other parallel
sequences alone. 

The `ANIMATION_PARALLEL` flag is always
implied when using a named sequence. However, a named sequence will
always supersede an earlier sequence with the same name, so you can\'t
have two sequences with the same name running concurrently.
