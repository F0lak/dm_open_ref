...
**Format:**
+   image(icon, loc, icon_state, layer, dir) (supports [named arguments](/ref/proc/arguments/named.md))
...
`loc`. For example, if `loc` is a mob, the image
will appear above the mob until it is destroyed. 

The `icon` argument can be anything that can be interpreted as an appearance, i.e.
...
The arguments `icon_state`, `layer`, and `dir` may be
...
Since the `loc` argument could never be a text string, the above statement can be
further shortened: 
```dm
