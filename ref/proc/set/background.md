...
If you have an eye for race conditions, you might think that the above code has one in which a mob gets deleted after it is assigned to M but before the call to M.Tick() is executed. However, <em>background processing is only interrupted at loop points in the code</em>, so the above code is safe. It would only ever be interrupted at the end of the <code>for</code> or <code>while</code> loops.
***
**Related Pages:**
