...
In older versions, the <a href="#/datum/proc/New">`New` proc</a> always had `waitfor` set to 0 by default, but this was later changed. Now 1 is always the default, so setting `waitfor` to 1 will result in a warning that it is no longer necessary.
***
**Related Pages:**
