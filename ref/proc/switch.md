...
Using the <a href="#/DM/preprocessor/pragma/syntax">`#pragma syntax` directive</a> you can change `switch()` to be more like other languages such as C. This replaces the if/else instructions with `case *value*` and `default` with a trailing colon. If you don't use the <a href="#/proc/break">`break` statement</a> at the end of a block, it will fall through to the next block.
...
Multiple cases can be put together with either commas between the values, or separate `case` statements. The `*A* to *B*` syntax is still allowed also.
***
**Related Pages:**
+    [if proc](/ref/proc/if)
