...
[Topic](/ref/client/proc/Topic/Topic.md){.code}), or any other such action.    
...
Certain built-in procs such as [atom/Click()](/ref/atom/proc/Click/Click.md){.code}    
are called automatically by a client counterpart like    
[client/Click()](/ref/client/proc/Click/Click.md){.code}; usually    
[atom/Stat()](/ref/atom/proc/Click/Click.md){.code} is called by    
[client/Stat()](/ref/client/proc/Click/Click.md){.code}; and so on. It is mostly    
...
proc such as [Move()](/ref/atom/movable/proc/Move/Move.md){.code} or    
[Enter()](/ref/atom/proc/Enter/Enter.md){.code}, because objs and non-player mobs    
may move autonomously without setting `usr`.    
Although `usr` is often set in [mob/Login()](/ref/mob/proc/Login/Login.md) when a    
...
[client.mob](/ref/client/var/mob/mob.md){.code}. It is safest to use `src` in    
`mob/Login()`, which is always correct, rather than `usr`.    
`usr` is the default point of reference for several procs like    
[view()](/ref/proc/view/view.md){.code} and [range()](/ref/proc/range/range.md){.code}, because    
of their common use in verbs. It is also the default recipient for    
[input()](/ref/proc/input/input.md){.code} and [alert()](/ref/proc/alert/alert.md){.code}    
