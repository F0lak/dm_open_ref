...
then you should use [`url_encode()`](/proc/url_encode) to encode
your command.
...
byondstorage
❌
Enables [`byondStorage`](/%7Bskin%7D/control/browser/byondStorage)
access.
### Example:
...
`winset()` or use [`url_encode()`](/proc/url_encode) to encode the
list of options. 
