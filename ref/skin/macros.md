...
[macro](/ref/%7Bskin%7D/param/macro/macro.md){.code} parameter.    
Macros can be changed at runtime. If a macro does not have an    
[id](/ref/%7Bskin%7D/param/id/id.md){.code}, you can refer to it by its key    
combination ([name](/ref/%7Bskin%7D/param/name/name.md){.code}). If you have a    
macro set named `macro1` and have a `Ctrl+E` macro for instance, you    
could use [winset()](/ref/proc/winset/winset.md){.code} with `"macro1.Ctrl+E"`. See    
the [Macro control](/ref/%7Bskin%7D/control/macro/macro.md) for information on which    
...
instant=1](/ref/verb/set/instant/instant.md){.code} on the verbs that will be tied to    
the Any macro, so that keyboard input doesn\'t queue up and lag behind.    
In the [command](/ref/%7Bskin%7D/param/command/command.md){.code} that goes with this    
...
The [map-to](/ref/%7Bskin%7D/param/map-to/map-to.md){.code} parameter is used by    
...
-   **D-pad:** The D-pad itself, which can be used to read a    
    [dir](/ref/atom/var/dir/dir.md){.code} number.    
...
`.gamepad-mapping` [command](/ref/%7Bskin%7D/commands/commands.md). Or, if they have    
...
[embedded wingets](/ref/%7Bskin%7D/commands/commands.md). These are the values you can    
...
with the [`ref()` proc](/ref/proc/ref/ref.md). It can be used as a verb argument    
...
they would behave as part of a [parameter list](/ref/proc/list2params/list2params.md).    
