...
used via its [`macro`](/%7Bskin%7D/param/macro) parameter.


Macros can be changed at runtime. If a macro does not have an
[`id`](/%7Bskin%7D/param/id), you can refer to it by its key
combination ([`name`](/%7Bskin%7D/param/name)). If you have a
macro set named `macro1` and have a `Ctrl+E` macro for instance, you
could use [`winset()`](/proc/winset) with `"macro1.Ctrl+E"`. See
...
instant=1`](/verb/set/instant) on the verbs that will be tied to
the Any macro, so that keyboard input doesn\'t queue up and lag behind.


In the [`command`](/%7Bskin%7D/param/command) that goes
...
The [`map-to`](/%7Bskin%7D/param/map-to) parameter is
...
[command](/%7Bskin%7D/commands). Or, if they have access to the Options
...
wingets](/%7Bskin%7D/commands). These are the values you can include in
a mouse macro:
...
reference such as you would get with the [`ref()` proc](/proc/ref). It
...
[parameter list](/proc/list2params). 
