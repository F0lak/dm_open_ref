...
turf/medal_door
  ...
  verb/Knock()
    usr << "Guard: Just checking your credentials...."
    var/hasmedal = world.GetMedal(medal, usr)
    if(hasmedal)
      usr << "Guard: Go right in."
      icon_state = "open"
      density = 0
    else if(!isnull(hasmedal))
      usr << "Guard: Sorry, no admittance without a [medal] badge."
    else
      usr << "Guard: Sorry, I lost the paperwork. Try again later."
